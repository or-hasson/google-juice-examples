...
import com.or.hasson.drawingapp.module.AppModuleProviderWithThread;
...
import com.or.hasson.drawingapp.requests.SquareRequestThread;


public class MainAppModuleWithProviderAndThread {
    ...
    private static void sendRequest(String sqaureReq) {
        ...
        if(sqaureReq.equals(SQAURE_REQ)){
            Injector injector = Guice.createInjector(new AppModuleProviderWithThread());
            SquareRequestThread request = injector.getInstance(SquareRequestThread.class);
            request.makeRequest();
        }
/**
